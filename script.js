...
function movePlayer(player, steps) {
    ...
    if (player.position + steps > tiles.length)
        player.money += 100;
...
function handleDiceRoll() {
    ...
    const rollResultDisplay = document.querySelector('.dice-roll-result');
            ...
            if (tiles[player.position].houseCost <= player.money) {
                ...
                if (currentTile) {
                    if (player.name === "player-1")
                        currentTile.style.border = "3px solid green"; // Example: Blue border
                    else
                        currentTile.style.border = "4px solid red";
                }
                updateMoney();
            }
            else
...
function handleBuyHotel(player) {
    ...
    if (returnOwner(player.position) === player) {
        if (tiles[player.position].type === 'property') {
            ...
            if (tiles[player.position].houseTier === 4) {
                ...
                alert("You don't have maxed out houses, can't buy hotel");
        }
        else
            alert("This is not buyable property");
    }
    else
    ...
    if (returnOwner(player.position) === player) {
        if (tiles[player.position].type === 'property') {
            ...
            if (tiles[player.position].houseTier <= 3) {
                ...
                alert("You have maxed out houses, can't buy anymore");
        }
        else
            alert("This is not buyable property");
    }
...
function updateMoney() {
    const moneyDivs = document.querySelectorAll('.money');
    moneyDivs.forEach((moneyDiv, i) => {
        moneyDiv.innerHTML = `Balance: ${players[i].money}`;
    });
}
function returnOwner(pos) {
    if (tiles[pos].owner) {
